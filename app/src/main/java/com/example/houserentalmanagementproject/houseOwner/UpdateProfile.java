...
public class UpdateProfile extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user != null) {
            ...
            mDatabase.child("Owner").child(userId).get().addOnCompleteListener(new OnCompleteListener<DataSnapshot>() {
                @Override
            ...
            copy_userID.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    Toast.makeText(com.example.houserentalmanagementproject.houseOwner.UpdateProfile.this, "User ID copied to clipboard", Toast.LENGTH_SHORT).show();
                }
            });
            btn_updateProfile.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    if (!newUsername.isEmpty() && !newEmail.isEmpty() && !newPhone.isEmpty()) {
                        mDatabase.child("Owner").child(userId).child("username").setValue(newUsername);
                        mDatabase.child("Owner").child(userId).child("userEmail").setValue(newEmail);
                        mDatabase.child("Owner").child(userId).child("search").setValue(newUsername.toLowerCase());
                        mDatabase.child("Owner").child(userId).child("phone").setValue(newPhone);

                        Toast.makeText(com.example.houserentalmanagementproject.houseOwner.UpdateProfile.this, "Change user information successful", Toast.LENGTH_SHORT).show();
                        finish();
                    } else {
                        Toast.makeText(com.example.houserentalmanagementproject.houseOwner.UpdateProfile.this, "Please fill out all fields", Toast.LENGTH_SHORT).show();
                    }
                }
            });
        }
